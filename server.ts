...
import { verifyCertificateFile } from './server/verification.js';
import { generateExcelReport, generatePdfReport, generateCsvReport } from './server/reports.js';
...
async function startServer() {
  ...
  app.post('/api/auth/login', (req, res) => {
    ...
    // Default lecturer credentials or matching email
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`SecureCert Verify server running on http://0.0.0.0:${PORT}`);
  });
}
